...
namespace Client.Services.Foundations.MedicalPlanningService
{
    ...
    public class MedicalPlanningService : IMedicalPlanningService
    {
        ...
        public async Task<List<PlanningDto>> GetAppointmentInformationPatientDoctorDto(KeysAppoimentInformationDoctor keysAppoimentInformationDoctor)
        {
            var DateAppoiment = System.Web.HttpUtility.UrlEncode(keysAppoimentInformationDoctor.DateAppoiment.Date.ToString());
    
            var CabinetId = System.Web.HttpUtility.UrlEncode(keysAppoimentInformationDoctor.CabinetId);
            var request = new HttpRequestMessage(HttpMethod.Get, $"/api/MedicalPlanning/ListAppoimentDoctorPatient/{CabinetId}/{DateAppoiment}");
            var JwtBearer = await this.localStorageServices.GetItemAsync<JwtDto>("JwtLocalStorage");
            request.Headers.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", JwtBearer.Token);
            var result = await httpClient.SendAsync(request);
            if (result.StatusCode == HttpStatusCode.OK)
